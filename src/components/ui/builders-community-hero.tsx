...
export interface CommunityOrbitProps {
  ...
  headline: ReactNode;
  /**
   * Element the headline renders as. Defaults to `h2` (the hero use, where the
   * headline is the section's heading). Pass `p` when the orbit sits under a
   * heading that already introduces the section, so the page does not end up
   * with two `h2`s in one block.
   */
  headlineAs?: 'h2' | 'p';
  tags?: OrbitTag[];
  minScale?: number;
...
export default function CommunityOrbit({
  ...
  const Headline = motion[headlineAs];
  const frameRef = useRef<HTMLDivElement>(null);
  const [scale, setScale] = useState(1);
  // The badges bob forever. For anyone who asked the OS for less motion that is
  // exactly the kind of animation to drop, so the float is stilled and the
  // entrance transitions collapse to nothing.
  ...
  return (
    // Background and ink are inherited from the host, not hardcoded here, so
    // the arc can sit on Mentorna's cream page instead of forcing a white band.
    <section className={`w-full px-4 pb-14 ${className ?? ''}`}>
      <div
        ref={frameRef}
        ...
        >
          ...
          <div className="absolute left-1/2 top-[393px] grid -translate-x-1/2 auto-cols-fr grid-flow-col gap-7">
            {stats.map((s, i) => (
              ...
              >
                <span className="text-[48px] font-medium leading-none tracking-[-0.02em] tabular-nums">
                  <CountUp value={s.value} delay={0.9 + i * 0.12} />
                </span>
                <span className="mt-[15px] text-[14px] leading-none opacity-55">{s.label}</span>
              </motion.div>
            ))}
          </div>
        </div>
