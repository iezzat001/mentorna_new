...
const SOURCE = 'Startup Problem Finder';
...
const ProblemFinder = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('magnet_leads').insert([leadData]);
      ...
      toast({
        title: 'تم بنجاح! 🎉',
        description: 'الـ Canvas في طريقه ليك.',
      });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      {/* ===== Lead Capture (moved above the analogies section) ===== */}
          ...
          <Card className="w-full max-w-lg mx-auto border-4 border-foreground shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white">
            <CardHeader className="bg-accent-yellow border-b-4 border-foreground text-center py-6">
              ...
              <CardTitle className="font-black text-xl md:text-2xl">
                {isSubscribed ? 'وصلك بنجاح! 🎉' : 'جاهز تطبق عملي؟'}
              </CardTitle>
              {!isSubscribed && (
                <p className="font-body text-sm text-foreground/80 mt-2">
                  نزّل الـ Problem-Finder Canvas (مجاناً) وامشي على الخطوات عشان تطلع بأول 3 مشاكل تقدر تبني
                  عليهم فكرتك.
                </p>
              )}
            </CardHeader>
                ...
                <div className="text-center py-4">
                  ...
                  <div className="border-t-4 border-dashed border-foreground/20 pt-5">
                    <p className="font-body text-sm text-foreground/70 mb-3">
                      لو ما وصلكش الـ{' '}
                      {contactMethod === 'whatsapp' ? 'رسالة على الواتساب' : 'إيميل'}، حمّل الـ Canvas من هنا 👇
                    </p>
                    <a
                      href={CANVAS_URL}
                      target="_blank"
                    ...
                    >
                      ...
                      تحميل الـ Problem-Finder Canvas
                    </a>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {contactMethod && (
                    <div className="space-y-4">
                      ...
                      >
                        {isSubmitting ? 'جاري الإرسال...' : 'ابعتلي الـ Canvas ←'}
                      </Button>
                      <button
