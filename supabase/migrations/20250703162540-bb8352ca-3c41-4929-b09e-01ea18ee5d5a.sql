...
-- Insert the 8 weeks with their basic information
INSERT INTO weeks (week_number, title, description, phase) VALUES
(1, 'Think Like an Entrepreneur', 'Discover how successful businesses start from simple ideas and learn to spot opportunities everywhere.', 'Foundation Building'),
(2, 'AI-Powered Business Planning', 'Master cutting-edge AI tools to research, analyze, and create professional business plans that impress.', 'Foundation Building'),
(3, 'Build Without Code', 'Create your first functional app or website using modern no-code tools - no programming required.', 'Foundation Building'),
(4, 'Customer Discovery Mastery', 'Learn to talk to real customers, gather feedback, and validate your ideas like a pro entrepreneur.', 'Foundation Building'),
(5, 'AI-Enhanced Product Development', 'Elevate your product using artificial intelligence for design, content, and advanced functionality.', 'Advanced Implementation'),
(6, 'Test, Iterate, Perfect', 'Master the art of user testing, feedback analysis, and rapid product improvement cycles.', 'Advanced Implementation'),
(7, 'Pitch Like a Pro', 'Develop compelling presentations and storytelling skills to confidently pitch your ideas to anyone.', 'Advanced Implementation'),
(8, 'Launch Your Business', 'Bring everything together and launch your first business with a real product demonstration.', 'Advanced Implementation')
...
-- Insert week outcomes
INSERT INTO week_outcomes (week_id, outcome_text)
SELECT w.id, outcome_text FROM week_ids w
JOIN (VALUES
...
) AS outcomes(week_num, outcome_text) ON w.week_number = outcomes.week_num
ON CONFLICT DO NOTHING;

-- Insert week activities for Week 1
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
) AS activities(week_num, activity_type, title, description, duration, order_index) ON w.week_number = activities.week_num
ON CONFLICT DO NOTHING;

-- Insert week activities for Week 2
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week activities for Week 3
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week activities for Week 4
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week activities for Week 5
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week activities for Week 6
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week activities for Week 7
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week activities for Week 8
INSERT INTO week_activities (week_id, activity_type, title, description, duration, order_index)
SELECT w.id, activity_type, title, description, duration, order_index
FROM week_ids w
JOIN (VALUES
...
-- Insert week skills for all weeks
INSERT INTO week_skills (week_id, skill_name, order_index)
SELECT w.id, skill_name, order_index
FROM week_ids w
JOIN (VALUES
...
) AS skills(week_num, skill_name, order_index) ON w.week_number = skills.week_num
ON CONFLICT DO NOTHING;
